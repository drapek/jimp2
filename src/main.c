	   ...
	   switch (opt) {
		  ...
		  case 'n': //digit indicating used n-grams
			 ...
			 if(tmp > 0 && tmp <= 20)
				...
				char * information[] = "Niepoprawnie podany ngram, dopszuczalne są z zakresu od 1 do 20, a podany został: ";
				error( 1, strcat(information, argv ), argc, argv );
			 }
			 break;

		  case 'd': //maximum quantity of produced word by text generator
			 tmp = atoi( optarg );
			 if(tmp > 0 && tmp <= MAX_INT) 
				FQUAWORD = tmp;
			 else {
				char * information[] = "Niepoprawnie podana liczba słów, dopszuczalne ilość słów waha się od 1 do 32767, a przez ciebie podana liczba to: ";
				error( 1, strcat(information, argv ), argc, argv );
			 }
			 break;
		  ...
		  case 't': //determine mode of program
			 ...
			 if( optarg[0] == 'p' ) 
				...
				FMODE = 1;
					...
					char * information[] = "Podałeś niepoprawny tryb programu, doposzczalne to s dla statystyki, oraz p dla produkcji (domyślnie),a ty podałeś: ";
				error( 1, strcat(information, argv ), argc, argv );
			 }
			 break;
		  ...
		  default:
			 ...
			 char * information[] = "Invalid parametr has been used! Check documentation before using!";
			 error( 1, information, argc, argv);
			 break;
